...
function Card({title,imageUrl,description,price,courseId}) {
  ...
  return (
    <div className='w-[24vw] h-[24vw] rounded-md border-2 border-gray-950 p-3'>
        <img src={imageUrl} alt={title} className='rounded-md h-[12vw] w-[22vw] bg-center object-cover' />
         ...
         <button onClick={()=>navigate(`/all/${courseId}/payment`)} className='w-full h-[3vw] bg-indigo-600 text-white rounded-md font-medium text-sm mt-2'>Add to Cart</button>
    </div>
  )
}
